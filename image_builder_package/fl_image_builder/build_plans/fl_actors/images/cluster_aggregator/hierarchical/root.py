...
# NOTE:
# The Root Aggregator (RAg) treats the Cluster Aggregators (CAg)
# as Learners/Clients. (Due to the current Flower limitations.)
class FLOpsHierarchicalRoot(fl.server.strategy.FedAvg):
    def __init__(
        self,
        aggregator_context: AggregatorContext,
        mlflow_experiment_id: int,
        requested_total_number_of_training_cycles: int,
        *args,
        **kwargs,
    ):
        init_logging()
        ...
        self.requested_total_number_of_training_cycles = (
            requested_total_number_of_training_cycles
        )
