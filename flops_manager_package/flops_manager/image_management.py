...
from flops_manager.ml_repo_management import MLRepoInfo, get_sanitize_ml_repo_name
...
def get_expected_image_repo_names_for_project(ml_repo_info: MLRepoInfo) -> List[str]:
    sanitized_image_repo_name = get_sanitize_ml_repo_name(ml_repo_info.url)
...
def check_if_latest_matching_images_exist(ml_repo_info: MLRepoInfo) -> bool:
    current_image_repo_names = get_current_registry_image_repo_names()
    expected_image_repo_names = get_expected_image_repo_names_for_project(ml_repo_info)
    if not all(
    ...
    for image_repo_name in expected_image_repo_names:
        ...
        expected_image_tag = ml_repo_info.latest_commit_hash
        if expected_image_tag not in current_matching_image_tags_in_registry:
            return False

    return True


def get_flops_image_name(ml_repo_info: MLRepoInfo, flops_image_type: FLOpsImageTypes) -> str:
    return "".join(
        (
            FLOPS_IMAGE_REGISTRY_IP,
            ":",
            FLOPS_IMAGE_REGISTRY_PORT,
            "/",
            get_sanitize_ml_repo_name(ml_repo_info.url),
            "/",
            flops_image_type.value,
            ":",
            ml_repo_info.latest_commit_hash,
        )
    )
