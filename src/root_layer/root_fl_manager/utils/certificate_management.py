...
# TODO find a better way of doing this - the optimal solution whould be a proper OAK CA but this does not seem to fit into my time budget
# So for not I use the mkcert utility that is not recommended to be used on prod systems
# TODO I generate these cerst here but primarily need them for the root image registy - I share them via volumes 
# Issue is that at the very first launch of these containers - where the volumes are empty but the registry container expect to find these certs
