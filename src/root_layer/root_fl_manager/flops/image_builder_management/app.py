...
def create_image_builder(
    ...
    logger.debug(f"Created builder SLA based on '{ml_repo.url}': {builder_sla}")
    ...
    response = custom_requests.CustomRequest(
        ...
        aux=custom_requests.RequestAuxiliaries(
            what_should_happen=f"Create new builder '{flops_process.flops_id}'-'{ml_repo.url}'",
    ...
    new_builder_app = next(
        (app for app in response if app["application_name"] == builder_name), None
    )
